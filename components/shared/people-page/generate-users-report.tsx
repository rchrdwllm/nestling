...
const reportFns: Record<string, () => Promise<any>> = {
  students: generateStudentsReport,
  instructors: generateInstructorsReport,
  admins: generateAdminsReport,
};
...
const GenerateUsersReport = () => {
  ...
  const handleDownloadCsv = async (data: any[], type: string) => {
    ...
    link.href = url;
    link.setAttribute("download", `${type}_report_${formattedDate}.csv`);
  ...
  const handleDownloadPdf = async (data: any[], type: string) => {
    ...
    doc.setFontSize(16);
    doc.text(`${type.charAt(0).toUpperCase() + type.slice(1)} Report`, 14, 18);
    doc.setFontSize(10);
    ...
    doc.save(`${type}_report_${safeDate}.pdf`);
  };
  ...
  const handleExport = async () => {
    ...
    try {
      const { success, error } = await reportFns[userType]();
      if (error || !success) {
        toast.dismiss();
        toast.error(
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button notAnimated variant="outline">
          <Share className="size-4" /> Export Users
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Choose a user type and format to export the data
          </DialogDescription>
        </DialogHeader>
